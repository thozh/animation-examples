          ...
          <TouchableOpacity
            ...
            <View>
              {key === 'delete' ? (
                ...
                <Text style={{ color: '#000', fontSize: 32, fontWeight: '700' }}>{key}</Text>
              )}
            </View>
          </TouchableOpacity>
        );
          ...
          <View
            ...
            {passcode[i] && (
              <MotiView
                ...
                animate={{
                  scale: isValid && passcode.length === passcodeLength ? [1.1, 1] : 1,
                  backgroundColor:
                    isValid && passcode.length === passcodeLength ? '#72C17F' : '#7a49a5',
                }}
...
export default function PassCodeV1() {
  ...
  return (
    <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
      ...
      <PassCode passcode={passcode} isValid={passcode.length !== passcodeLength || isValid} />
      <PassCodeKeyboard
        onPress={(char) => {
          if (char === 'delete') {
            console.log('delete');
            setPasscode((passcode) =>
              passcode.length === 0 ? [] : passcode.slice(0, passcode.length - 1)
            );
            return;
          }
          if (passcode.length === passcodeLength) {
